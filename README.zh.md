...
- 1. 在data下传入标注好的数据集:eet_events.json  在dict下传入本体标签数据文件:event_schema.json

- 2. sh bin/script/data_preparation4retrain.sh  预处理数据

- 3. sh bin/script/train_and_eval.sh  训练和预测

     

#### 预测步骤：

- 1. 在data下传入已经处理好的每行只包含text,id的json文件:test_temp.json 

- 2. sh bin/script/train_and_eval4test.sh 开始预测 生成pred_role.json在result文件夹下  
