...
def origin_events_4test(sents, file_name):
	"""
	...
	# TODO 2020-11-23
	target_path = u"{0}/{1}".format(save_dir, file_name)
	# print(output)
	if output:
		...
		if os.path.exists(target_path):
			...
			print("++++++++++++++++remove exist file: " + target_path)
		with open(target_path, 'w', encoding='utf-8') as fw:
			pass


def handle_return_events_4test(sentences):
	"""
	origin_events_process
	deal test json data like:{"text": "国际组织IEEE限制华为后，北大教授宣布退出IEEE编委会", "id": "17388c65ef9d77aff12d8cc1f645e41c"}
	返回处理后的列表
	"""
	sents = format_test_lst(sentences)
	output = []
	for d_json in sents:
		event = {}
		event["trigger"] = ""
...
# TODO 2020-11-23
def save_test_dict_file(sentences, file_name):
	"""
	将字符串列表转换两次格式并保存为预测需要的文件格式 test.json
	"""
	res = format_test_lst(sentences)
	# save to test.json
	origin_events_4test(res, file_name)
	

# def read_pred_json_format():
# TODO 2020-11-23
def read_pred_json_format(pred_path):
	"""
...
def read_pred_json_format_1126(outputs):
	"""
	读取已经预测完成的pred.json文件并格式化为需要的json格式返回给客户端
	"""
	res = []
