...
time0 = time.time()
print('time0---------------------------------->耗时{:.2f}'.format(time0 - start_time))

def main(args):
    """main"""
    time1 = time.time()
    print('time1---------------------------------->耗时{:.2f}'.format(time1 - time0))
    ...
    if args.do_train:
        ...
        # 208
        log.debug("Device count: %d" % dev_count)
        log.debug("Num train examples: %d" % num_train_examples)
        log.debug("Max train steps: %d" % max_train_steps)
        log.debug("Num warmup steps: %d" % warmup_steps)
        ...
        log.debug("Theoretical memory usage in training: %.3f - %.3f %s" %
    ...
    if args.do_train:
        time2 = time.time()
        print('time2---------------------------------->耗时{:.2f}'.format(time2 - time1))
        ...
        if args.init_checkpoint:
            ...
            init_pretraining_params(
                ...
                use_fp16=args.use_fp16)
        time3 = time.time()
        print('time3---------------------------------->耗时{:.2f}'.format(time3 - time2))
    ...
    if args.do_train:
        ...
        while True:
            try:
                ...
                # log.debug("train pyreader queue size: %d" % (train_pyreader.queue.size()))
                log.debug('This is debug one steps == %d ' % steps)
                ...
                if steps % args.validation_steps == 0:
                    # evaluate dev set
                    log.debug('This is debug steps == %d ' % steps)
                    if args.do_val:
                ...
                log.debug('Train end steps == %d' % steps)
    ...
    if args.do_test:
        time4 = time.time()
        print('time4--------------------------------------------->耗时{:.2f}'.format(time4 - time3))
        ...
        time5 = time.time()
        print('time5---------------------------------------------->耗时{:.2f}'.format(time5 - time4))
